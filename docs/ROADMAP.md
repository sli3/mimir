...
| 54-HOTFIX | DF4/DF5 ICAO trust gate (TD-54-6 fix) + pyModeS pin to ==3.3.0 — Added _trusted_icaos cache (300s TTL, per-ICAO, refreshed on genuine DF17/18) to AdsbDecoder; DF4/DF5-derived ICAOs are only accepted if the same ICAO has been seen in a CRC-valid DF17/18 extended squitter within the last 300 seconds. Pinned pyModeS to exactly ==3.3.0 in pyproject.toml because the gate's correctness depends on 3.3.0's specific crc_valid semantics (bare df membership test); 3.5+ changes crc_valid to None for DF4/5 without an icao_hint, which Mimir's existing `not result.get("crc_valid", False)` check treats as falsy, silently rejecting every DF4/DF5 frame. Live-traffic confirmation: 7C68CE on Pluto, clean DF17 traffic (typecodes 11 and 19), zero DF4/5 noise or singleton-ICAO spam. Commits: 815f394 (feat) + 5b50cce (chore/deps). | ✅ Complete | 1136 (815 pytest + 321 Vitest), 0 failures |
| 53-HOTFIX | ADS-B frame timestamp coercion at the radar trail boundary — server.py:666 emits ADS-B frame timestamp as ISO 8601 string via msg.timestamp.isoformat(); mergeAircraftRecord.js passes it through unchanged; RadarScopePanel.jsx was doing const ts = ac.timestamp ?? Date.now() at two call sites — the ?? only catches null/undefined, so the trail buffer stored strings. Trail then did string arithmetic (ts - last.ts > TRAIL_STALE_MS, newest.ts - oldest.ts inside derivePredictionVector) which produced NaN. NaN comparisons are always false. Consequences, all three verified empirically: (1) pathPrediction.js derivePredictionVector() returned null on every live frame — the physics readout, the amber projected ghost line, and the Phase 53 "ANALYSE PATH WITH LLM" button were all permanently unreachable against live data. Phases 52 and 53 had never once worked in production. (2) The trail staleness check never fired — trails silently bridged across arbitrarily long dead time, the exact behaviour Phase 50 was designed to prevent. (3) Trail points still accumulated (the push gate last.ts !== ts is a string inequality that works), producing a misleading partial-success signature: visible trail, dead prediction. The fix: new parseFrameTs.js helper (44 lines) coerces frame timestamp to numeric epoch ms — ISO string via Date.parse, numeric passthrough, Date.now() fallback for null/undefined/NaN/garbage so a malformed timestamp can never poison the trail buffer with NaN. RadarScopePanel.jsx uses it at both call sites. ISO wire format was deliberately NOT changed — six other consumers legitimately consume the string form (AisVesselPanel, AcarsMessagePanel, App.jsx "LAST SEEN", AIReasoningPanel, SignalHistoryLog, VectorSpacePage). Test changes: 20 breadcrumb-trail fixture sites in RadarScopePanel.test.jsx converted from numeric timestamp: to ISO 8601 strings (intervals preserved, including the 60s/95s/200s/1000ms gaps the existing tests assert on). 2 new tests added in new describe('Phase 53-HOTFIX — ISO timestamp coercion', ...) block: a seam regression test driving the real RadarScopePanel with ISO timestamps, reading back from the resulting trailsRef, and calling the real derivePredictionVector from utils/pathPrediction.js (asserting non-null vector with expected rates); and a staleness-clearing test asserting a +95s ISO-timestamped gap clears the polyline. 8 new test cases in new parseFrameTs.test.js: ISO parse, ISO subtraction arithmetic, numeric passthrough, null/undefined/garbage-string fallback, NaN/Infinity fallback, and 11-input finiteness sweep. Seam regression test verified to FAIL against the unfixed code before the fix was applied. Mocked-seam lesson: both sides of the seam were tested with numeric fixtures, and the seam itself was mocked on both sides — RadarScopePanel.test.jsx fed numeric timestamp: values; PathPredictionPanel.test.jsx constructed trail history directly into a mock trailsRef and never drove RadarScopePanel's real accumulation logic. The producer was tested with numbers, the consumer was tested with numbers, and nothing exercised the real connection between them with the real payload type. Green tests, broken feature — a concrete instance of the "green tests are necessary but insufficient" principle, specific to mocked seams between two individually-correct units. Live verification: confirmed working on real Adelaide air traffic post-fix (ADALM-PLUTO, 1090 MHz, passive receive only). Aircraft RXA4846 / ICAO 7C81D6 rendered a populated detail card, a live physics readout in the prediction strip, and a successful LLM verdict ("Steady climb on a stable heading", HIGH CONFIDENCE) — the first time the Phase 52 physics readout and the Phase 53 LLM analysis button have ever functioned against live data. No TD row for this bug (found+fixed same session = bookkeeping theatre). Known follow-on: fixing this exposed a second, distinct bug one function downstream in pathPrediction.js (compute elapsedSec without /1000 divisor), previously invisible because derivePredictionVector never got past its NaN guard on real data — thetaDegPerSec and deltaRNmPerSec are 1000x too small, ghost line is 1000x too conservative. This will be addressed as Phase 52-HOTFIX. +10 net Vitest-only (8 in parseFrameTs.test.js, 2 in RadarScopePanel.test.jsx). pytest unchanged. | ✅ Complete | 1146 (815 pytest + 331 Vitest), 0 failures |
| 52-HOTFIX | ms→s unit correction in derivePredictionVector — variable `elapsedSec` was actually holding milliseconds (trail ts is epoch ms from parseFrameTs.js, no /1000 existed), so thetaDegPerSec and deltaRNmPerSec were 1000× too small; invisible before Phase 53-HOTFIX because the ISO string timestamp never got past the NaN guard. Fix: rename elapsedSec→elapsedMs, derive elapsedSec = elapsedMs/1000 AFTER the validity guard, document the unit contract in module docstring + JSDoc. **Behaviour change (not a regression): llm/path_reasoner.py's _HIGH_TURN_RATE_DEG_PER_SEC = 3.0 turn-rate threshold in _high_turn_rate_flagged() becomes live in production for the first time since Phase 53 — previously all live rates arrived 1000× too small.** 7 derivePredictionVector fixtures converted to ms-scale; 2 new regression tests added (ms→s strict equality + realistic 1-second ADSB scenario). projectPosition() and PREDICTION_HORIZON_SEC unchanged. **Queued follow-on (separate phase): server.py theta bound widening (-30, 30) at risk of close-overhead pass with corrected rates; LlmReasoningPanel 400→cause:'network' mislabel fix.** +2 Vitest only. | ✅ Complete | 1148 (815 pytest + 333 Vitest), 0 failures |
| 55 | Radar prediction panel bundle (Bearing/Range field, prediction glyph, theta bound widening, 400→rejected mapping, prediction panel sizing) — `dashboard/server.py` widened `_REASON_NUMERIC_RANGES["theta_deg_per_sec"]` from (-30.0, 30.0) to (-90.0, 90.0) with multi-line inline comment; `AircraftDetailPanel.jsx` added new `formatBearingRange(bearing, range)` helper and filled 4th static-column slot with bearing/range, header docstring updated; `LlmReasoningPanel.jsx` imported PredictionGlyph, added `ERROR_MESSAGES.rejected` entry, branched cause in postReasonRequest (400→'rejected' else 'network'), placed glyph above verdict in result JSX; `RadarPage.css` increased `.radar-prediction-panel` height 70px→400px, verdict 12px→22px with wrap, notes 10px→16px with line-clamp 6, confidence 9px→13px, added `.prediction-glyph` and `.prediction-glyph-pulse` rules, added `@keyframes prediction-pulse`, added `prefers-reduced-motion` disable. New files: `PredictionGlyph.jsx` (141 lines, pure presentational SVG component with 2 solid + 3 ghost dots, dashed connector, rotation clamped to ±45° from vector.thetaDegPerSec, CSS-only pulse, React.memo, data-testid="prediction-glyph"). Tests: `AircraftDetailPanel.test.jsx` added 4 new test cases, `LlmReasoningPanel.test.jsx` added 3 new test cases, `test_radar_reason_endpoint.py` added TestThetaBoundWidened class with 8 test cases. +22 net (+8 pytest, +14 Vitest). | ✅ Complete | 1170 (823 pytest + 347 Vitest), 0 failures |
| 57 | Receiver reference position fix — `modules/adsb/constants.py` `ADELAIDE_LAT`/`ADELAIDE_LON` updated from CBD placeholder (-34.93, 138.60) to actual receiver location (-34.92290, 138.61047); comment block rewritten to accurately describe the live BearingTracker → AdsbSubscriber → /radar broadcast chain. Backend Python file only — zero frontend import surface, Vitest not re-run. +0 net (823 pytest passing unchanged, 347 Vitest unchanged). | ✅ Complete | 1170 (823 pytest + 347 Vitest), 0 failures |
| 58 | Floating θ/Δr box + continuous anomaly strip + bottom panel redesign + post-build manual corrections — `RadarScopePanel.jsx` adds a 3-line detail box (ICAO/callsign, θ, Δr) for the selected aircraft in place of the plain callsign label; non-selected contacts keep the plain label; box reuses the existing ghost-line vector computation (no duplicate `derivePredictionVector`/`projectPosition` call); anchor side flips on θ's sign so the box clears the ghost line; Δr renders at 2dp because 1dp rounded every real contact to "0.0nm/s"; `Number.isFinite` guard on both vector fields with ICAO-only box fallback (ADV-02). `PathPredictionPanel.jsx` adds a continuous anomaly flag strip — emergency squawk (7500/7600/7700), rapid altitude change (\|vertical_rate\| > 3000 ft/min), high turn rate (\|θ\| > 3.0 deg/s) — that renders on selection with no LLM click required; squawk and altitude flags render during the "gathering history" state, only turn-rate waits for a resolved motion vector. `LlmReasoningPanel.jsx` has `PredictionGlyph` moved out into the main column as a sibling (glyph top-left, verdict/confidence/notes underneath); orphan physics readout line removed. Threshold contract: frontend owns its own `HIGH_TURN_RATE_DEG_PER_SEC` constant rather than fetching it at runtime; new contract test `tests/llm/test_path_reasoner_thresholds.py` reads `_HIGH_TURN_RATE_DEG_PER_SEC` out of `llm/path_reasoner.py` and asserts it matches the JS value, so drift fails at build time. Stale comments corrected: `llm/path_reasoner.py` and `LlmReasoningPanel.jsx` both had false "squawk is always None, flag never fires" claims removed (TD-53-A and TD-54-1). Post-build manual corrections by Prin: `BOX_PAD_X = 6` inset in `RadarScopePanel.jsx`; `RadarPage.css` `.radar-prediction-panel` height 400px→300px, verdict 22px→18px, notes 16px→13px, `.radar-prediction-llm-result` gained 6px 8px padding, `.radar-anomaly-strip` restyled from a bordered-left sidebar to a self-contained floating box (own border, background, radius, margin; align-items flipped flex-end→flex-start; `align-self: flex-start` added to stop the box stretching to the full panel height). @frontend-reviewer returned empty on both Phase 58 and Phase 58-FIX runs (TD-58-C). +19 net (+1 pytest, +18 Vitest). | ✅ Complete | 1189 (824 pytest + 365 Vitest), 0 failures |
| 58-FIX-4 | Three live-testing UI fixes to the /radar bottom prediction panel — `RadarPage.css` Fix 1: `.radar-prediction-llm-idle` `justify-content` flex-end→flex-start so the "ANALYSE PATH WITH LLM" button sits flush left. Fix 2: `.prediction-glyph` `margin-bottom: 2px`→`-28px` plus `overflow: visible` to close the empty vertical gap between the SVG glyph and the LLM verdict text — the 90-unit viewBox is deliberately tall so the ±45° rotation clamp keeps dot-row endpoints inside (y≈2.6..87.4) at extreme bearing rates, so the viewBox was kept unchanged and the negative margin does the cosmetic work; `overflow: visible` overrides the SVG's UA-default `overflow: hidden` to prevent the ~1.4px dot-edge sliver clip at the clamp extremes. Fix 3: `.radar-prediction-panel` flipped from `display: grid; grid-template-columns: minmax(0, 3fr) minmax(0, 1fr); align-items: stretch;` to `display: flex; flex-direction: column; gap: 8px;` (2-column grid removed; panel is now single-column); new `.radar-prediction-glyph-row` class with `display: flex; align-items: flex-start; gap: 12px;`; `PathPredictionPanel.jsx` state 3 JSX restructured to wrap `<PredictionGlyph>` and `{anomalyStrip}` together in `.radar-prediction-glyph-row` at the top of `.radar-prediction-main`, with `<LlmReasoningPanel>` full-width underneath; state 2 JSX UNCHANGED. `PredictionGlyph.jsx` untouched. Two follow-up hand-edits from the same live-testing pass: `padding: 0 12px` added to `.radar-prediction-placeholder`/`.radar-prediction-gathering` (previously unset — panel padding itself was untouched), and `height: 100%` added to the same two rules so the placeholder text vertically centres inside the flex-column panel (previously pinned to the top border because flex-column default `justify-content: flex-start` made padding horizontal-only). 2 new structure tests in `PathPredictionPanel.test.jsx` verify the new state-3 glyph-row wrapper and confirm state-2 strip is a direct child of the panel. Dual-reviewed (@review-second + @deep-analyst, both approved); @frontend-reviewer returned empty for the third consecutive /radar run (TD-58-C, escalation recommended). Live-verified by Prin on real ADS-B traffic on 2026-08-07 across multiple screenshots, including a two-flag anomaly case (rapid altitude climb + high turn rate simultaneously). +2 Vitest, +0 pytest. | ✅ Complete | 1191 (824 pytest + 367 Vitest), 0 failures |
| 59 | Ghost-dot direction-indicator rework on RadarScopePanel — reworked selected-aircraft IIFE ghost-dots from three true-to-scale 15s/30s/45s positional projections into a fixed-length normalised vector (33 px, 0.22×SCOPE_MAX_R) with three evenly-spaced amber dots at data-position="1"|"2"|"3"; degenerate guard (trueLength < 0.01) suppresses dots when displacement sub-pixel; existing dashed line and θ/Δr box preserved. Live-traffic fixes: (1) line/dot alignment via shared lineStart/lineEnd endpoint (7C7772 exposed divergence); (2) ring-clearance + length fix via RING_CLEARANCE_PX = 8 offset and 0.15→0.22 multiplier bump (7C6DB4 exposed dot1 merging with selection ring); (3) box-direction fix via real computed direction (dx, dy) instead of theta sign alone — `boxOnLeft = !v || dx >= 0` (7C389F/7C2EB8 exposed box overlapping line). TD-58-C RESOLVED: frontend-reviewer fixed via opencode.json explicit Playwright verb allows (gitignored, local-only) + model swap to opencode-go/gpt-5.6-luna. Verified via two-part diagnostic (static-only + live-browser) — both paths now produce real output. 2 frontend files touched + 1 test file + agent definition. Dual-reviewed (@review-second and @deep-analyst both approved); frontend-reviewer verified working. Live-verified by Prin on real ADS-B traffic (same contacts as 58-FIX-4). +10 Vitest, +0 pytest. | ✅ Complete | 1201 (824 pytest + 377 Vitest), 0 failures |
| 62 | Real spectral fingerprint measurements attached to captures. `capture_and_save()` gained a required `band` parameter, validated against `BAND_PROFILES` before any hardware call (ValueError on unrecognised band, band never guessed from `freq_hz`). After capture, `compute_psd()` then `fingerprint_spectrum()` (both unchanged) measure the captured samples using the band's `signal_threshold_db`, `crop_half_width_hz`, and `burst_use_wide_window`, falling back to `fingerprint_spectrum()`'s own defaults via `.get()` when a band profile does not define one of those keys. `save_capture()` gained an optional `fingerprint` parameter; when provided, exactly seven of `fingerprint_spectrum()`'s fifteen returned keys (`peak_freq_hz`, `peak_power_db`, `noise_floor_db`, `snr_db`, `bandwidth_hz`, `occupied_bins`, `spectral_flatness`) are written as a single nested `mimir:fingerprint` SigMF global field via the `_FINGERPRINT_METADATA_KEYS` constant; the other eight (`center_freq_hz` plus seven detection-pipeline internals: `signal_threshold_db`, `snr_margin_db`, `peak_bin_power_db`, `burst_ratio_db`, `expected_noise_ratio_db`, `burst_excess_db`, `is_burst`) are excluded and stay internal to the function. When `fingerprint` is `None` the field is omitted entirely, not written as null or empty. No software DSP is applied to the saved samples; this is measurement only. `compute_psd()`, `fingerprint_spectrum()`, and `BAND_PROFILES` were not modified. 9 existing tests in `test_capture_pipeline.py` updated to pass the new required `band` argument; 4 new tests added (28 passed / 0 failed in `test_capture_pipeline.py`, full pytest 841 passed / 0 failed, Vitest unchanged at 377 passed / 0 failed from Phase 61). +4 pytest, +0 Vitest. | ✅ Complete | 1218 (841 pytest + 377 Vitest), 0 failures |
| 63 | SNR-edge auto-capture trigger (armable, per-band). New `_should_fire_trigger()` pure helper in `core/pipeline/scanner.py`; new trigger check block inside `ScanRunner._scan_loop()` between `fingerprint_spectrum()` and `embed()` step; uses `save_capture(samples, freq_hz, sample_rate_hz, device=self._device_driver, fingerprint=fingerprint)` with no `bandwidth_hz`; wrapped in `try/except Exception`. `_scan_loop` docstring gained step 7. `dashboard/server.py` gained new `handle_set_capture_trigger` SocketIO handler below `handle_set_focus`. Defensive non-raising: AttributeError for non-dict, isinstance(str) + frozenset check, strict bool coercion for `armed`. Does NOT touch focus or current_band. `dashboard/shared_state.py` gained new `TRIGGER_ARMABLE_BANDS` frozenset (fm_broadcast, aprs, ism, adsb); new `trigger_armed` dict, `_trigger_last_snr` dict, `trigger_state_lock`, four helper functions. Tests: `tests/core/test_scanner.py` (+188) with new `TestShouldFireTrigger` (10 unit) + `TestScanLoopCaptureTrigger` (5 integration); `tests/dashboard/test_server_stats.py` (+80) with new `TestSetCaptureTrigger` (13 handler tests); `tests/dashboard/test_shared_state.py` (+102) with new `TestCaptureTriggerState` (10 state helper tests). +38 pytest (+0 Vitest). | ✅ Complete | 1256 (879 pytest + 377 Vitest), 0 failures |
| 64 | LIFE-01 (disarm clears `_trigger_last_snr`) + EDGE-03 (trigger reuses fingerprint pipeline's `threshold` local, not redundant `band.get('signal_threshold_db')`). Tests: `test_set_trigger_armed_false_clears_last_snr` + `test_set_trigger_armed_true_does_not_clear_last_snr` (TestCaptureTriggerState, 2 tests), `test_missing_threshold_key_uses_shared_fallback_on_both_paths` (TestScanLoopCaptureTrigger, 1 test). +3 pytest (+0 Vitest). | ✅ Complete | 1259 (882 pytest + 377 Vitest), 0 failures |
| 64b | TOCTOU-HELPER (atomic check-and-update helper, Path B: armed-gated None-safe conditional write) + LOW-02 (_should_fire_trigger docstring note); LOGGER-NONE-ARM (defensive logger ternary in trigger-fire logger.info, kept as defensive symmetry; not touched in 64b). Files: dashboard/shared_state.py (lines 511-560 helper), core/pipeline/scanner.py (trigger block + docstring). Tests: 4 new in TestCaptureTriggerState. +4 pytest (+0 Vitest). | ✅ Complete | 1263 (886 pytest + 377 Vitest), 0 failures |
...
- **LIFE-01: stale SNR survives disarm/re-arm** — `_trigger_last_snr` is now cleared on `armed=False` transition in `set_trigger_armed()`; re-arm compares against fresh SNR only. *(Phase 64)*
- **EDGE-03: trigger threshold re-read duplicating pipeline value** — `trigger_threshold_db` read removed from `_scan_loop()`; now reuses the existing `threshold` local variable carrying fallback default. *(Phase 64)*
- **TOCTOU-HELPER: gap in trigger's per-cycle armed-check + prev-SNR + new-SNR sequence** — Added atomic `check_and_update_trigger_state()` helper in `dashboard/shared_state.py` (lines 511–560) that reads `was_armed` and `prev_snr` in one critical section, and conditionally writes `current_snr` only when `was_armed AND current_snr is not None` (Path B implementation). Original spec called for unconditional write; amended mid-build after test pollution regression caught in TestScanLoopDeviceGuard. `ScanRunner._scan_loop()` now calls the helper instead of three separate `is_trigger_armed()` / `get_last_trigger_snr()` / `set_last_trigger_snr()` calls. *(Phase 64b)*
- **LOW-02: stale docstring clause in `_should_fire_trigger()`** — Added "Note: after Phase 64'\''s EDGE-03 fix..." paragraph to the existing "Returns False whenever any input is None..." paragraph in `core/pipeline/scanner.py`'\''s `_should_fire_trigger()` docstring. No logic or signature change. *(Phase 64b)*
...
### Phase 55 — Radar prediction panel bundle (Bearing/Range field, prediction glyph, theta bound widening, 400→rejected mapping, prediction panel sizing) ✅

**Type:** Backend + frontend. UI enhancement + validation refinement + visual projection.

**Goal.** Enrich the /radar prediction panel with (a) a combined bearing/range static field in AircraftDetailPanel, (b) a directional prediction glyph showing the derived vector, (c) widened theta_deg_per_sec validation bounds on the /api/radar/reason endpoint (±30→±90) to accommodate close-overhead passes, (d) HTTP 400 validation rejection mapped to a distinct "rejected" error message (not "network"), and (e) enlarged panel height (70px→400px) so LLM reasoning reads as a primary readout.

**What was built.**

**Backend:**

- `dashboard/server.py` — `_REASON_NUMERIC_RANGES["theta_deg_per_sec"]` widened from (-30.0, 30.0) to (-90.0, 90.0) with multi-line inline comment explaining the bound's job is prompt-injection defence, not physical plausibility filtering. The comment documents that >30°/s occurs inside ~0.16 nm at 300kt groundspeed, so ±90 discharges the prompt-injection duty identically while removing false-positive risk on legitimate close overhead passes.

**Frontend:**

- `dashboard/frontend/src/components/AircraftDetailPanel.jsx` — added new `formatBearingRange(bearing, range)` helper that zero-pads bearing to three characters and renders range to one decimal; filled the 4th static-column slot (previously blank) with bearing/range; updated header docstring to reflect "bearing/range - 4 fields" in the static column. If either input is missing the whole field falls back to em-dash.

- `dashboard/frontend/src/components/LlmReasoningPanel.jsx` — imported PredictionGlyph; added `ERROR_MESSAGES.rejected` entry; branched cause in postReasonRequest (400→'rejected' else 'network') with multi-line comment explaining that 400 validation rejection is now an operator-visible condition and must not be disguised as a connectivity failure; placed glyph above verdict in result JSX.

- `dashboard/frontend/src/components/PredictionGlyph.jsx` — NEW pure presentational SVG component (141 lines). Shows the derived prediction vector as a horizontal row of five dots joined by a dashed connector: two solid cyan history dots on the left, three amber ghost projection dots on the right. Angle derived from `vector.thetaDegPerSec` clamped to ±45°, applied as rotation about the centre dot. Rotation contract: 1 deg/s of bearing rate = 1 degree of visual deflection, capped at +/-45 degrees. Wrapped in React.memo; CSS-only pulse via `.prediction-glyph-pulse` keyframes; includes `prefers-reduced-motion` disable. `data-testid="prediction-glyph"` for testing. Renders nothing when vector is null.

- `dashboard/frontend/src/pages/RadarPage.css` — increased `.radar-prediction-panel` height 70px→400px with comment explaining the LLM reasoning output now reads as a primary readout; verdict font-size 12px→22px with wrap; notes font-size 10px→16px with line-clamp 6; confidence font-size 9px→13px; added `.prediction-glyph` and `.prediction-glyph-pulse` rules; added `@keyframes prediction-pulse`; added `@media (prefers-reduced-motion: reduce)` disable.

**Tests:**

- `tests/dashboard/test_radar_reason_endpoint.py` — added TestThetaBoundWidened class with 8 test cases covering theta bounds at 60°, 45°, 89.99°, 90.0° (accepted), and 90.1°, -90.1°, NaN, missing key (all rejected). +8 pytest.

- `dashboard/frontend/src/tests/AircraftDetailPanel.test.jsx` — added 4 new test cases: bearing/range field rendering, zero-padding single-digit bearing, em-dash for null bearing, em-dash for null range. +4 Vitest.

- `dashboard/frontend/src/tests/LlmReasoningPanel.test.jsx` — added 3 new test cases: glyph renders above verdict, 400 maps to "Invalid request — payload rejected", 500 maps to "LLM unreachable". +3 Vitest.

- `dashboard/frontend/src/tests/PredictionGlyph.test.jsx` — NEW test file with 7 test cases covering null vector, zero theta, positive theta, negative theta, clamping at ±45°, NaN theta, and deltaRNmPerSec unused. +7 Vitest.

**Key design decisions.**

- Combined bearing/range field placement: The 4th static-column slot was the correct location — it groups identity fields (callsign, ICAO, squawk, bearing/range) together on the left, keeping dynamic per-frame fields (altitude, track, groundspeed, vertical rate) on the right.

- Glyph angle derivation: thetaDegPerSec maps naturally to visual rotation (angular motion around the receiver); deltaRNmPerSec was deliberately unused because it describes range change with no natural rotation mapping. Documented as a future extension point.

- Pulse animation: CSS-only (no JS timers) to avoid React state churn and to respect prefers-reduced-motion. The marker lives inside the rotated group so the pulse travels along the projected direction.

- 400→rejected mapping: A 400 validation rejection is now an operator-visible condition (since widened theta bound means a 400 can occur on a legitimate close overhead pass), not a Mimir-side programming error. It must not be disguised as a connectivity failure.

- Panel height enlargement: The fixed-height rule is unchanged — content swap only, never a resize. The 70px→400px change makes LLM reasoning a primary readout (Mimir is an LLM-powered monitoring tool first), not a footnote.

**Resolved tech debt.** None.

**New tech debt.** TD-55-1 logged: `AircraftDetailPanel.jsx`'s local `formatBearingRange` helper does not apply `% 360` the way the shared `formatBearing()` in `utils/aircraftFormat.js` does. A bearing of exactly 360° would render as "360°" instead of "000°". Unreachable from `BearingTracker` today (its `atan2` output is already normalised to [0, 360)), so this is a consistency gap rather than a live bug. Fix: reuse `formatBearing()` rather than reimplementing it.

**Test counts.** 1170 passing (823 pytest + 347 Vitest), 0 failures. +22 net (+8 pytest, +14 Vitest). Verified by Prin directly running `pytest` from repo root and `npm run test` from `dashboard/frontend/`.

**RF/Legal notes.** No TX surfaces; all changes are UI display logic on already-decoded ADS-B data and validation refinement. No new RF capability or hardware interaction. Jurisdiction: AU/SA, ACMA, Radiocommunications Act 1992 (Cth). ADS-B is legal to receive passively at 1090 MHz in Australia.

---

### Phase 58-FIX-4 — Three live-testing UI fixes to the /radar bottom prediction panel ✅

**Type:** Frontend only. UI polish corrections after live testing.

**Goal.** Fix three layout issues discovered during Prin's live testing on real ADS-B traffic on 2026-08-07: (a) the "ANALYSE PATH WITH LLM" button in the idle state was flush right instead of flush left, (b) a noticeable vertical gap existed between the PredictionGlyph SVG and the LLM verdict text in state 3, and (c) the anomaly flag strip in state 3 was not visually associated with the glyph (it sat in the sidebar from the old 2-column grid layout). Two follow-up hand-edits from the same live-testing pass improved placeholder vertical alignment and panel padding.

**What was built.**

**Frontend:**

- `dashboard/frontend/src/pages/RadarPage.css` — Fix 1: `.radar-prediction-llm-idle` `justify-content` flipped from `flex-end` to `flex-start`, so the "ANALYSE PATH WITH LLM" button sits flush left in the idle state (matching the operator's expectation that the button anchors to the prediction panel's left edge). Fix 2: `.prediction-glyph` `margin-bottom` changed from `2px` to `-28px` and `overflow: visible` added to close the empty vertical gap between the SVG glyph and the LLM verdict text. The 90-unit viewBox is deliberately tall so the ±45° rotation clamp keeps dot-row endpoints inside (y≈2.6..87.4) at extreme bearing rates; the viewBox was kept unchanged and the negative margin does the cosmetic work. `overflow: visible` overrides the SVG's UA-default `overflow: hidden` to prevent the ~1.4px dot-edge sliver clip at the clamp extremes. Follow-up hand-edit A: `padding: 0 12px` added to `.radar-prediction-placeholder` and `.radar-prediction-gathering` (both previously had no padding property) for horizontal breathing room around the placeholder text. `.radar-prediction-panel`'s own padding was not touched. Follow-up hand-edit B: `height: 100%` added to both `.radar-prediction-placeholder` and `.radar-prediction-gathering` so the placeholder text vertically centres inside the flex-column panel (previously pinned to the top border because flex-column default `justify-content: flex-start` made padding horizontal-only).

- `dashboard/frontend/src/components/PathPredictionPanel.jsx` — Fix 3: state 3 JSX restructured. The `.radar-prediction-panel` flipped from a 2-column grid (`display: grid; grid-template-columns: minmax(0, 3fr) minmax(0, 1fr); align-items: stretch;`) to a single-column flex container (`display: flex; flex-direction: column; gap: 8px;`). A new `.radar-prediction-glyph-row` class with `display: flex; align-items: flex-start; gap: 12px;` wraps both `<PredictionGlyph>` and `{anomalyStrip}` together at the top of `.radar-prediction-main`, with `<LlmReasoningPanel>` full-width underneath. State 2 JSX is unchanged — the anomaly strip remains a direct child of the panel in the gathering state, per Prin's explicit call to allow visual relocation between states. `PredictionGlyph.jsx` is untouched.

**Tests:**

- `dashboard/frontend/src/tests/PathPredictionPanel.test.jsx` — 2 new structure tests verify the new state-3 glyph-row wrapper (asserting both glyph and strip are children of the flex wrapper) and confirm state-2 strip is a direct child of the panel (not nested in a row). +2 Vitest.

**Key design decisions.**

- Button alignment flip: The idle-state button now sits flush left (`flex-start`) to match the operator's expectation that the button anchors to the prediction panel's left edge. This is a cosmetic correction discovered during live testing.

- Glyph gap closure via negative margin: The 90-unit viewBox is deliberately tall to accommodate ±45° rotation clamping; changing the viewBox would break the rotation safety margin. The negative margin approach is purely cosmetic and does not affect the SVG's rendering contract. `overflow: visible` is required to prevent the ~1.4px dot-edge sliver clip at the clamp extremes.

- 2-column grid → single-column flex: The original 2-column grid layout (glyph column, reasoning column) made the anomaly strip visually associated with the reasoning column rather than the glyph. Moving to a single-column flex with a glyph-row wrapper groups the glyph and strip together visually, while still allowing full-width reasoning underneath. State 2 keeps the strip as a sibling below the main container to preserve the "gathering" semantics.

- Follow-up hand-edits from same live-testing pass: `padding: 0 12px`, added to `.radar-prediction-placeholder` and `.radar-prediction-gathering` (previously unset on both), gives the text better horizontal breathing room without increasing the overall panel height. The `height: 100%` on placeholder classes ensures the text vertically centres inside the flex-column panel, which was previously pinned to the top border due to flex-column's default `justify-content: flex-start`.

**Resolved tech debt.** TD-58-A RESOLVED: PathPredictionPanel State 3 (LLM result state) is now live-verified across multiple real ADS-B contacts on 2026-08-07, including the idle-state button placement, the glyph-to-verdict gap close, the anomaly strip beside-glyph layout in a two-flag case (rapid altitude climb + high turn rate simultaneously), and the placeholder vertical-centre fix. Both "No aircraft selected" placeholders now correctly clear of all borders and vertically centred. The 300px height fits the verdict+confidence+notes without overflow as built.

**New tech debt.** TD-58-B updated: Item 1 (`.radar-prediction-gathering { grid-column: 1 / -1 }` being dead in State 2) RESOLVED by this phase — the rule was changed to `width: 100%` as a no-op equivalent on the new flex-column parent. Two LOW advisory items remain. TD-58-C updated: `@frontend-reviewer` returned empty for the third consecutive /radar run (fourth data point if counting Phase 53). Port-conflict theory was tested and ruled out — reviewer still returned empty after the port was cleared and Vite confirmed running cleanly on 5173. TD-58-D updated: Reminder that `dashboard/static/assets/*` is stale relative to source — operator action before next serve.

**Test counts.** 1191 passing (824 pytest + 367 Vitest), 0 failures. +2 Vitest, +0 pytest. Verified by PM before finalise.

**RF/Legal notes.** No TX surfaces; all changes are CSS/JSX only — no Python, no physics, no hardware interaction. No new RF capability or legal exposure. Jurisdiction: AU/SA, ACMA, Radiocommunications Act 1992 (Cth).
...
Real spectral fingerprint measurements attached to captures. `capture_and_save()` (`core/pipeline/capture.py`) gained a required `band` parameter, validated against `BAND_PROFILES` before any hardware call (`ValueError` on an unrecognised band; the band is never guessed from `freq_hz`, the caller must state it explicitly).

After capture, the existing `compute_psd()` and `fingerprint_spectrum()` (both unchanged) measure the captured samples using the band's `signal_threshold_db`, `crop_half_width_hz`, and `burst_use_wide_window`, falling back to `fingerprint_spectrum()`'s own defaults via `.get()` when a band profile does not define one of those keys.

`save_capture()` gained an optional `fingerprint` parameter. When provided, exactly seven of `fingerprint_spectrum()`'s fifteen returned keys (`peak_freq_hz`, `peak_power_db`, `noise_floor_db`, `snr_db`, `bandwidth_hz`, `occupied_bins`, `spectral_flatness`) are written as a single nested `mimir:fingerprint` SigMF global field via the `_FINGERPRINT_METADATA_KEYS` constant. The other eight keys (`center_freq_hz` plus seven detection-pipeline internals: `signal_threshold_db`, `snr_margin_db`,
`peak_bin_power_db`, `burst_ratio_db`, `expected_noise_ratio_db`, `burst_excess_db`, `is_burst`) are excluded and stay internal to the function. When `fingerprint` is `None` the field is omitted entirely, not written as null or empty.

No software DSP is applied to the saved samples — this is measurement only. `compute_psd()`, `fingerprint_spectrum()`, and `BAND_PROFILES` were not modified.

9 existing tests in `test_capture_pipeline.py` were updated to pass the new required `band` argument; 4 new tests were added (28 passed / 0 failed in `test_capture_pipeline.py`). Full pytest: 841 passed / 0 failed. Vitest unchanged at 377 passed / 0 failed from Phase 61 (`dashboard/frontend/` was not touched this phase).

---

### Phase 63 Detail

SNR-edge auto-capture trigger (armable, per-band). New `_should_fire_trigger()` pure helper in `core/pipeline/scanner.py` implements edge detection: fires when `prev_snr < threshold <= current_snr`, where threshold is the band's own `signal_threshold_db` with no fallback default (see tech-debt EDGE-03). Re-arms naturally once SNR drops back below threshold, so a continuous strong signal produces one capture at the rising edge, not one per scan cycle.

`_scan_loop()` gained a new trigger check block between the `fingerprint_spectrum()` call and the `embed()` step. If the current band is in `TRIGGER_ARMABLE_BANDS` and armed, and `_should_fire_trigger()` returns true, the block calls `save_capture(samples, freq_hz, sample_rate_hz, device=self._device_driver, fingerprint=fingerprint)` with no `bandwidth_hz` — omitted because HackRF has no settable RF filter and, on Pluto, a live-loop capture simply inherits whatever the stream's filter already is, so there is no operator-declared width to record. The call is wrapped in `try/except Exception` so a failed save (e.g. disk full) is logged and never halts the live scan loop. `_scan_loop` docstring gained step 7 describing the trigger check.

`dashboard/server.py` gained a new `handle_set_capture_trigger` SocketIO handler below `handle_set_focus`, using the same defensive, non-raising style: a non-dict payload, a non-string band, or a band outside `TRIGGER_ARMABLE_BANDS` is logged and ignored rather than raised. `armed` is coerced with strict-bool-only logic — any non-`bool` value becomes `False`. The handler does not modify `handle_set_focus` behaviour or touch `current_band` in `shared_state.py`.

`dashboard/shared_state.py` gained a new `TRIGGER_ARMABLE_BANDS` frozenset (`fm_broadcast`, `aprs`, `ism`, `adsb` — the four bands carrying a real `# Calibrated:` comment on `signal_threshold_db`; `aviation`/`acars`/`ais` are deliberately excluded as not field-verified); new `trigger_armed` and `_trigger_last_snr` dicts (default `False`/`None` per armable band); a shared `trigger_state_lock`; and four helper functions: `set_trigger_armed(band, armed)`, `is_trigger_armed(band)`, `get_last_trigger_snr(band)`, and `set_last_trigger_snr(band, snr_db)`.

Tests: `tests/core/test_scanner.py` (+188 lines) with new `TestShouldFireTrigger` (10 unit tests on the pure helper) and `TestScanLoopCaptureTrigger` (5 integration tests confirming `save_capture()` is actually called from inside a real `_scan_loop()` run on a crossing, not just that the pure helper returns true in isolation). `tests/dashboard/test_server_stats.py` (+80 lines) with new `TestSetCaptureTrigger` (13 handler tests). `tests/dashboard/test_shared_state.py` (+102 lines) with new `TestCaptureTriggerState` (10 state-helper tests).

Plan-reviewer issued a BLOCK mid-build: the original plan targeted `dashboard/capture_loop.py`, which turned out to be dead code (unimported by `scan.py` or `server.py`). Resolved by wiring the trigger into the real live path, `ScanRunner._scan_loop()`, instead.

Pytest baseline was 841 → 879 (+38 net, all from Phase 63's new tests). Vitest unchanged at 377 (frontend not touched this phase). Combined total: 1218 → 1256.
