"""Tests for AisSubscriber — lifecycle, queue, frequency filter, scan loop integration."""
...
class TestAisSubscriber:
    def test_subscriber_ignores_non_ais_frequency(self):
        """IQ chunks at 98 MHz are silently dropped."""
        calls = []
        sub = AisSubscriber(broadcast_fn=lambda m: calls.append(m))
        sub.receive(np.zeros(100, dtype=np.complex64), 98_000_000.0, 2_000_000.0)
        assert sub._queue.empty()

    def test_subscriber_accepts_ais_frequency(self):
        """IQ chunks at 162.000 MHz are queued."""
        calls = []
        sub = AisSubscriber(broadcast_fn=lambda m: calls.append(m))
        sub.receive(np.zeros(100, dtype=np.complex64), 162_000_000.0, 2_000_000.0)
        assert not sub._queue.empty()

    def test_subscriber_accepts_within_tolerance(self):
        """IQ chunks within 100 kHz of 162.000 MHz are queued."""
        calls = []
        sub = AisSubscriber(broadcast_fn=lambda m: calls.append(m))
        sub.receive(np.zeros(100, dtype=np.complex64), 162_050_000.0, 2_000_000.0)
        assert not sub._queue.empty()

    def test_subscriber_drops_when_queue_full(self):
        """When queue is full, new chunks are dropped without exception."""
        sub = AisSubscriber(broadcast_fn=lambda m: None)
        # Fill the queue
        for _ in range(32):
            sub.receive(np.zeros(100, dtype=np.complex64), 162_000_000.0, 2_000_000.0)
    ...
    def test_scan_loop_broadcasts_to_subscriber(self):
        ...
        assert received[0][1] == 162_000_000.0
