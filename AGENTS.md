...
**Current phase:** 60 — SigMF Rewrite of `save_capture()` / `capture_and_save()` (Part A only). Part B (separate build, separate prompt) will add Pluto support and the `bandwidth_hz` override. The `_DEVICE_HW_NAMES` dict is now structured to make that addition trivial. `core:extensions` declaration is a SigMF spec requirement for custom namespaces; suppresses a DeprecationWarning today and prevents a future ValidationError. Hardware verification performed: live HackRF One at 98 MHz FM Adelaide confirmed the write path works against real hardware, not only synthetic arrays. 1 new content round-trip test (`test_metadata_round_trips_device_and_legal_provenance`) added; pyproject.toml added `"sigmf>=1.11.0"` dependency; uv.lock resolved to `sigmf==1.11.1` + `defusedxml==0.7.1`. `capture_iq()` and `capture_iq_pluto()` untouched. +1 pytest, +0 Vitest.

**Current total:** 1200 passing (823 pytest + 377 Vitest), 2 failures (pre-existing Track A test debt — see Known Tech Debt table for root causes and one-line fixes).
