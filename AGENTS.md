...
| Corrected `crop_half_width_hz` for `ais` (50_000) — HIGH-01 FIXED 2026-07-13 | Original 12_500 half-width (window 161.9875–162.0125 MHz) cropped the dead gap between the two AIS channels and excluded both: CH1 161.975 MHz and CH2 162.025 MHz each sit ±25 kHz from the BAND_PROFILES dual-channel midpoint 162.000 MHz, double the original half-width. Corrected to 50_000 (window 161.950–162.050 MHz), covering both channels with margin. Fix applied same session in `dashboard/shared_state.py`. Value is corrected but still NOT field-verified — revisit once a real AIS vessel packet is captured live to confirm the width. | Field verification |
| Placeholder `crop_half_width_hz` for `aviation` (12_500) | Estimated from 25 kHz VHF voice channel spacing convention (half of 25 kHz). Not field-verified — needs a live air-traffic capture to confirm. Centre 127 MHz is on a real aviation channel; 12.5 kHz half-width should be correct for a single-channel centre, unlike the AIS case. One-line fix in `dashboard/shared_state.py` once a real capture validates. | Field verification |
| Placeholder `crop_half_width_hz` for `acars` (12_500) | Same reasoning as aviation — 25 kHz VHF channel spacing convention. Not field-verified; needs a live ACARS packet capture to confirm. Centre 129.125 MHz is the AU ACARS primary frequency. One-line fix in `dashboard/shared_state.py` once a real capture validates. | Field verification |
| Placeholder `crop_half_width_hz` for `ism` (250_000) | BAND_PROFILES centre 915.000 MHz does NOT land on a real AU915 channel (AU915 starts at 915.2 MHz; channels are 125 kHz or 500 kHz wide). The 2 MHz capture window likely spans 4–5 real channels at once, so no single-channel crop value is intrinsically right. 250_000 chosen as half of the widest AU915 channel (500 kHz) — conservative. Not field-verified; needs a capture during confirmed live LoRa traffic. One-line fix in `dashboard/shared_state.py` once a real capture validates. | Field verification |
| Placeholder `crop_half_width_hz` for `adsb` (900_000) | Deliberately conservative. Sources disagree on ADS-B/Mode S occupied bandwidth (~1 MHz vs ~2 MHz). Full capture window is 2 MHz — aggressive crop risks clipping real pulse energy. 900_000 half-width (1.8 MHz window) stays inside the ~1 MHz prior estimate from `tools/diagnose_threshold.py`'s `target_bw_hz=1_000_000` with a small margin. Not field-verified; needs a live-aircraft capture with the spiral discone to confirm. One-line fix in `dashboard/shared_state.py` once a real capture validates. | Field verification |
| Live scanner vs tool embedding space mismatch (Phase 30) | Live scanner (`ScanRunner._scan_loop`) now forwards `crop_half_width_hz` from BAND_PROFILES to `fingerprint_spectrum()`; the 5 offline tools (`tools/seed_chromadb.py`, `tools/capture_to_vectorstore.py`, `tools/calibrate_thresholds.py`, `tools/diagnose_threshold.py`, `tools/diagnose_fingerprints.py`) still call with default `None` (uncropped). For single-signal captures the difference is zero; for multi-signal captures the 7D embedding has up to 5 dimensions that differ (`peak_freq_hz`, `peak_power_db`, `snr_db`, `bandwidth_hz`, `occupied_bins`), biasing L2 distance upward between cropped live fingerprints and uncropped stored vectors. User spec explicitly deferred tool changes; fix is to thread `crop_half_width_hz` from BAND_PROFILES into `tools/capture_to_vectorstore.py` and `tools/seed_chromadb.py` and re-ingest, then optionally re-tune `_DISTANCE_SCALE_REFERENCE` in `llm/classifier.py`. | Future phase |
