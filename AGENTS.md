...
| TD-54-6 (medium-to-high priority — NEW FINDING FROM LIVE TESTING, not present at build time) | Live testing on the ADALM-PLUTO device immediately following this phase's deployment showed a DF4/DF5 frame volume noticeably higher than DF17 volume, which is the opposite of expected ADS-B traffic shape (DF17 self-broadcast should normally dominate over DF4/DF5 ground-radar-reply traffic). Observed pattern: one ICAO producing a high rate of repeated frames, alongside a long tail of distinct ICAOs each appearing exactly once and never recurring. This pattern is more consistent with noise/malformed-frame CRC collisions being decoded as spurious DF4/DF5 hits than with genuine SSR reply traffic, though this has NOT been confirmed — only observed via the live dashboard UI, not yet measured with real per-DF counts. This is NOT a regression introduced by Phase 54's code — Phase 54 only added the capability to see DF4/DF5 at all; the underlying RF environment and pyModeS's decode behaviour are unchanged. This is flagged as an open investigation for a dedicated future session, not a defect to fix in this finalise. Likely next step: extend tools/compare_decode_rate.py (already built, already uses the real production decode path) with a per-df tally in DecodeStats, to get real counts instead of impression-based UI observation. | Future dedicated investigation phase (field/desk hybrid) |
