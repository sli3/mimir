...
**Current phase:** 68 + 69 (combined finalise) — "Record" start/stop capture (Phase 68) and waterfall event markers (Phase 69). Phase 68 ships `ScanRunner.start_recording()`/`stop_recording()`/`get_recording_status()`, `save_recording()` in `core/pipeline/capture.py`, POST `/api/record/{start,stop}` routes, `RecordButton.jsx`, and `useRecording.js` — coexists with the pre-existing one-shot `capture_now()` without modifying it. Phase 69 ships three event-marker types (Capture Now / Record start / Record stop) drawn on the EXISTING crosshair overlay canvas in `WaterfallPanel.jsx`, plus a new `useWaterfallMarkers.js` hook. Frontend-only, no backend changes. Both phases live-verified against PlutoSDR at 915.825 MHz (488.6 MB SigMF pair, 466 cycles) with independent GNU Radio cross-tool validation confirming standards-compliant SigMF output. Phase 69 live-verified on ADS-B waterfall strip with all three marker styles visible and legible. Commits: 16a6d0c (Phase 68 backend), 98b15cb (Phase 68 UI), 66885d8 (Phase 69). +10 tests (Phase 69), 0 regressions.
...
**Current total:** 1365 passing (954 pytest + 411 Vitest), 0 failures.
