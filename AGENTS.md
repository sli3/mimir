...
| Pluto band profiles uncalibrated | `PLUTO_BAND_PROFILES` gain_db (30.0) and signal_threshold_db (3.0) values remain PROVISIONAL — placeholders copied from the HackRF profiles, not calibrated for Pluto. gain 30.0 was chosen from a spur observation, not a calibration session. Phase 39 ships `capture_iq_pluto()` + `tools/diagnose_pluto_gain.py` for structured sweep interpretation; awaiting operator hardware-sweep run using the new tool, then a follow-up edit / Phase 39b. Pluto must not become the default device until this closes. | Phase 39b |
