...
**Current phase:** 76 — "Demo Mode (--demo flag) for scan.py, Complete Arc". `python scan.py --demo --demo-files <sigmf-file>` runs the full, genuinely live, interactive dashboard against a looping replayed SigMF capture with no SDR hardware attached and no live network dependency on the yubaba LLM server (cached LLM responses instead). Three new components: `core/pipeline/demo_producer.py` (DemoProducer — daemon-thread producer that replays SigMF chunks via the new `core/pipeline/fingerprint.py:fingerprint_from_psd`, paces itself via `DEMO_CHUNK_INTERVAL_SEC=0.05s` independent of `config.dwell_time_sec`), `llm/demo_classifier.py` (DemoSignalClassifier — subclasses SignalClassifier, serves pre-generated cached responses instead of live HTTP), `tools/generate_demo_cache.py` (one-time cache-generation tool). scan.py's --demo flag bypasses SDR hardware initialisation and ACARS/AIS/ADS-B decoder subscribers (scope boundary — see below). After the initial build, FIVE live-verified bugs surfaced that 1022+ passing tests could not catch; fixes landed across both source commits. Test counts: 1515 total passing (1042 pytest + 473 Vitest), 0 failures. Zero source behaviour changes in this finalise — governance docs only.

**Phase 75** — "Replay stats visual card layout (frontend-only)". Visual-only follow-up to Phase 74. Two changes: (1) Timeline strip removed entirely from the burst analysis panel (.replay-burst-timeline JSX block deleted from ReplayPage.jsx, associated CSS rules .replay-burst-timeline, .replay-burst-timeline-seg, .replay-burst-timeline-seg:hover deleted from ReplayPage.css, two timeline-specific tests deleted from ReplayPage.test.jsx, comments cleaned up). Rationale: at real capture sizes (332-474+ chunks), the timeline rendered as hundreds of near-invisible slivers directly above the chunk grid, which already provides the same colour-coded view, click-to-select, and chunk numbering. Confirmed via live screenshot comparison. (2) Stats panel restyled from compact single-line text into a 4-column grid of cards (each card has a label div via .replay-burst-stat-label and a value div via .replay-burst-stat-value). Full-range card has replay-burst-stat-card-secondary modifier class; only the label is dimmed (via CSS, .replay-burst-stat-card-secondary .replay-burst-stat-label) — an earlier inline color: var(--text-dim) style on the value itself was removed during live review after it read as an unexplained inconsistency with no attached meaning, so only the label now signals the secondary stat. BurstStats useMemo block (lines 277-323 of ReplayPage.jsx) unchanged. OneShotResult untouched. Zero backend files touched. Test counts: 1423 total passing (973 pytest + 450 Vitest), 0 failures. Vitest net -1: 2 timeline tests removed + 1 structural card test added + 1 assertion updated.
...
**Current total:** 1515 passing (1042 pytest + 473 Vitest), 0 failures.
...
| [Phase 76] TD-76-1 — DemoProducer uses the first file's freq/sample_rate/band/device for ALL files in a multi-file --demo-files list (no per-file resolution). LOW severity — single-file demos, the tested and documented case, work correctly. | DemoProducer reads metadata from the first SigMF file in the --demo-files list and applies those freq_hz, sample_rate, band, and device values to ALL files in the list. No per-file resolution or validation. Single-file demos (the tested and documented case) work correctly; multi-file playlists spanning different bands/devices are not representable in the current data model (see TD-76-8). | Future phase |
| [Phase 76] TD-76-2 — DemoSignalClassifier.classify() could theoretically raise on a deeply malformed cache shape; _load_cache() doesn't catch RecursionError. LOW severity, unrealistic given the cache is machine-generated by generate_demo_cache.py. | DemoSignalClassifier.classify() trusts the cache file shape entirely (assumes it's a dict with chunk-index keys and value dicts with 'classification', 'confidence', 'reasoning', 'au_legal_status', 'confidence_score'). A deeply malformed cache (e.g., circular references via pathological JSON) could raise RecursionError during json.load(), which _load_cache() does not catch. LOW severity — the cache is machine-generated by generate_demo_cache.py, which produces well-formed output; this is a theoretical risk, not an observed one. | Future phase |
| [Phase 76] TD-76-3 — Dashboard focus-click can silently freeze SIGNAL HISTORY during demo if a non-demo band is clicked. LOW/cosmetic, recoverable by reselecting the demo's own band. | In demo mode, if the operator clicks a band button that differs from the demo file's actual band (e.g., demo file is ADS-B at 1090.03 MHz, operator clicks FM at 98.306 MHz), the backend's _focused_freq_hz variable changes to the clicked band's rounded canonical value. DemoProducer continues emitting items with the real captured frequency (1090.03 MHz), which no longer matches the new focus, so broadcast() filters them out. SIGNAL HISTORY and AI REASONING stop updating, though the waterfall continues (broadcast_spectrum has no focus filter). Recoverable by clicking the correct band button again. LOW/cosmetic, but confusing. | Future phase |
| [Phase 76] TD-76-4 — The unchanged-verdict throttle (Phase 43, unchanged_emit_interval_sec=5.0) applies identically in demo mode, meaning noise-classified chunks' cache entries become dead weight in generate_demo_cache.py's output (LLM calls paid for chunks that will rarely or never emit). | Phase 43's unchanged-verdict throttle (unchanged_emit_interval_sec=5.0) applies identically in demo mode: if consecutive chunks classify identically (e.g., both "noise" with identical reasoning), only the first is emitted within a 5-second window. When generate_demo_cache.py is run later to build the demo cache, it still pays LLM API calls for ALL chunks, but the throttled chunks' cache entries become dead weight — they will rarely or never be emitted during a demo because the throttle suppresses them at broadcast time. This is a cost/efficiency consideration for future demo-cache tooling, not a functional bug. | Future phase |
| [Phase 76] TD-76-5 — core/pipeline/replay.py retained a few now-unused imports (compute_psd, fingerprint_spectrum, resolve_band_profile) after the fingerprint_samples() extraction to core/pipeline/fingerprint.py. TRIVIAL, confirmed NOT a duplication risk (replay.py correctly imports and calls the new shared function, does not carry a duplicate copy) — just dead imports. | After Phase 76 extracted fingerprint_samples() from core/pipeline/replay.py into the new core/pipeline/fingerprint.py module, replay.py retained three imports (compute_psd, fingerprint_spectrum, resolve_band_profile) that are no longer used in that file. TRIVIAL — confirmed NOT a duplication risk (replay.py correctly imports and calls fingerprint_from_psd() from the new shared module, does not carry a duplicate copy of the fingerprinting logic). These are just dead imports that should be cleaned up. | Future phase |
| [Phase 76] TD-76-6 — Demo mode's SCAN COUNT and ACTIVE FREQUENCY stats stay at 0/--- throughout a demo session, because these are set exclusively inside _scan_loop, which demo mode correctly never runs. Cosmetic — the CLASSIFIED counter and AI REASONING panel are the meaningful demo-mode indicators instead. | In demo mode, ScanRunner._scan_loop never runs (DemoProducer is the producer instead), so SCAN COUNT stays at 0 and ACTIVE FREQUENCY shows "---" throughout the session. These stats are set exclusively inside _scan_loop. This is correct behaviour — demo mode does not scan — but it can be confusing to operators. The CLASSIFIED counter and AI REASONING panel are the meaningful demo-mode indicators instead. Cosmetic/aesthetic issue only. | Future phase |
| ~~[Phase 76] TD-76-7 — Decode-path (/radar, ADS-B raw-frame decode, RAW DECODE / FRAME INSPECTOR panels) remains entirely out of --demo mode's scope by design. A future phase would need a genuinely different producer (raw IQ samples, not fingerprints) feeding AdsbSubscriber. NOT currently scoped or scheduled.~~ | ~~Mimir has TWO separate ADS-B-related pipelines. (1) The fingerprint/classify path — what --demo mode drives — produces the AI REASONING panel's classifier verdict. (2) A completely separate DECODE path (AdsbSubscriber -> pyModeS -> real bit-level frame decoding -> aircraft ICAO/callsign/position -> the RAW DECODE and FRAME INSPECTOR panels, and the /radar page's aircraft tracks) — this needs RAW IQ SAMPLES, which the fingerprint-only demo producer does not provide. This was explicitly, deliberately scoped OUT of Phase 76 from the very first planning conversation — confirmed at the end of this arc, when live testing showed RAW DECODE and FRAME INSPECTOR correctly staying empty ("Awaiting frames...") while AI REASONING correctly populated. THIS IS NOT A BUG. It is Phase 76's designed scope boundary. /radar and true ADS-B decode replay remain a legitimate, separate FUTURE phase, not yet scoped or scheduled.~~ | ~~Future phase (separate, unscheduled)~~ (RESOLVED 2026-09-03: commit 4edbb108 added AdsbDemoProducer (core/pipeline/adsb_demo_producer.py), a daemon-thread producer that replays a SigMF capture as raw IQ into AdsbSubscriber.receive(), running alongside the existing fingerprint/classify DemoProducer. scan.py's --demo branch wires it via the new optional --demo-files-adsb flag, default data/captures/capture_1090030000hz_20260820_153307.sigmf-meta (gitignored). Live-verified: two full wrap cycles with zero error lines; missing explicit path exits with code 1.) |
| [Phase 76] TD-76-8 — Multi-band demo playlists (multiple --demo-files spanning different bands) are not representable in DemoProducer's current single-band-per-instance data model (relates to TD-76-1). | DemoProducer's current data model assumes a single band per instance: it reads freq/sample_rate/band/device from the first file and applies those values to ALL files. Multi-band demo playlists (e.g., --demo-files adsb.sigmf fm.sigmf) are not representable in this model — the producer would need per-file metadata resolution and potentially per-file focus management. Related to TD-76-1 (which notes the single-file case works correctly). | Future phase |
| [Phase 76] TD-76-9 — Symmetric JS-side parity test missing — `dashboard/frontend/src/tests/frequency.test.js:84-88` already asserts the JS FREQ_TOLERANCE_HZ value, but no JS-side test reads the Python FOCUS_FREQ_TOLERANCE_HZ constant and asserts parity; only the Python→JS direction is pinned today (via `tests/core/test_frequency.py:test_python_tolerance_matches_frontend_constant`). | The cross-language contract test `test_python_tolerance_matches_frontend_constant` (tests/core/test_frequency.py:83-121) pins the Python FOCUS_FREQ_TOLERANCE_HZ constant (core/pipeline/frequency.py) to the JS FREQ_TOLERANCE_HZ constant (dashboard/frontend/src/utils/frequency.js) — Python reads the JS value and asserts equality. This is one-directional (Python→JS). The reverse direction (JS reads Python and asserts parity) does not exist yet. The JS test file `dashboard/frontend/src/tests/frequency.test.js:84-88` already asserts that FREQ_TOLERANCE_HZ === 100_000, but it doesn't read the Python constant. A symmetric test would catch drift from the JS side. | Future phase |
| [Phase 76] TD-76-10 — dashboard/frontend/src/utils/frequency.js's docstring name-collision warning may still only mention the ADS-B constant rather than all three siblings — see Step 8 above; if @doc-writer's pass did not resolve this, log it here instead. (RESOLVED 2026-08-31: @doc-writer confirmed in this finalise that the collision warning now enumerates all three sibling constants — ADS-B 2 MHz, AIS 100 kHz, ACARS 5 kHz — with the AIS trap explicitly flagged.) | RESOLVED this finalise — @doc-writer confirmed that the docstring name-collision warning in dashboard/frontend/src/utils/frequency.js now enumerates all three sibling constants (modules/adsb/constants.py: 2_000_000 for ADS-B, modules/ais/constants.py: 100_000 for AIS, modules/acars/constants.py: 5_000 for ACARS), with the AIS trap (same numeric value as the new frontend/backend focus-matching constant) explicitly flagged. No action needed. | RESOLVED this finalise |
| [Phase 76] TD-76-11: AdsbDemoProducer's constructor exception contract is incomplete (float(meta.sample_rate) can raise an undocumented SigMFAccessError if core:sample_rate is missing from a future file). LOW priority, consistent with the pre-existing DemoProducer pattern. | The AdsbDemoProducer constructor (core/pipeline/adsb_demo_producer.py) calls float(meta.sample_rate). The sigmf library raises SigMFAccessError when a global core field is absent, but the constructor docstring documents only FileNotFoundError and ReplayFileError, so a future SigMF file lacking core:sample_rate would surface an undocumented exception type. Same latent shape as the existing DemoProducer constructor, so low priority. | Future phase |
| [Phase 76] TD-76-12: Final partial chunk is discarded on each file wrap; ~0.2% sample loss at worst, negligible. | The _run loop in core/pipeline/adsb_demo_producer.py wraps to start_index=0 whenever start_index + chunk_samples would exceed total_samples (the deterministic EOF pre-check), so the samples between the last full ADSB_DEMO_CHUNK_SAMPLES chunk and the end of the file are never replayed. A frame straddling the file tail is lost once per loop. Negligible for demo purposes. | Future phase |
| [Phase 76] TD-76-13: The scan.py smoke test proves wiring, not concurrency. | test_scan_demo_starts_both_producers in tests/core/test_adsb_demo_producer.py patches both scan.DemoProducer and scan.AdsbDemoProducer, so "neither producer blocks the other" is only established at the mock level, not under real concurrent load. Low priority. | Future phase |
| [Phase 76] TD-76-14: AdsbSubscriber's internal queue has a 64-message residency ceiling; bounded and drop-on-full. | modules/adsb/subscriber.py constructs queue.Queue(maxsize=64) and receive() uses put_nowait, logging a debug line and dropping the chunk on queue.Full. Identical semantics to live mode, not a regression, but worth noting alongside the other queue-related rows already in the table. | Future phase |
| [Phase 76] TD-76-15: Default-missing graceful degradation not live-verified end to end (genuine gap, NEW 2026-09-03). | scan.py's "warning and skip ADS-B when the default file is absent" branch has unit test coverage (test_missing_default_adsb_demo_path_graceful_degradation, tests/scan/test_demo_main.py, mocked) but was NOT live-verified end to end in the TD-76-7 session: Prin only had one working demo cache locally, for the canonical file which is also the ADS-B default, so isolating "default missing, fingerprint still valid" as a live run would have required generating a second cache. Unit coverage exists; live coverage of this branch does not yet. | Future session with two demo caches available |
