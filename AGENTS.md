...
**Current total:** 1270 passing (893 pytest + 377 Vitest), 0 failures.
...
| ADS-B signal_threshold_db now stale against max-hold trace (HIGH-01) | HackRF `BAND_PROFILES["adsb"].signal_threshold_db` (3.0 dB) and Pluto `PLUTO_BAND_PROFILES["adsb"].signal_threshold_db` (8.0 dB) were calibrated against the averaged `psd_db` trace, before Phase 65 Fix B switched the live trigger to `psd_max_hold_db`. Max-hold sits on a systematically higher floor, so both threshold values are now stale relative to what the live loop actually measures. `tools/diagnose_threshold.py` is also still hardcoded to `trace_key='psd_db'` for its own recommendation sweep, so re-running it as-is will not produce a max-hold-correct number — that tool needs its own `trace_key` plumbing fix first. Blocks trustworthy live verification of the SNR-edge auto-capture trigger until resolved. Suggested next step: extend `trace_key` support to `diagnose_threshold.py`'s `BAND_SWEEP`/CLI, then re-run `--band adsb --device hackrf` and `--band adsb --device pluto` against confirmed live traffic. | Phase post-65 (diagnose_threshold.py plumbing fix first, then live re-sweep) |
