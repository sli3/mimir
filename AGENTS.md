...
**Current phase:** 61 - Pluto device support + bandwidth_hz for `capture_and_save()` (Part B) + Track A debt clearance. Part B added ADALM-PLUTO device support to `capture_and_save()` in `core/pipeline/capture.py`. New `device: str = "hackrf"` parameter accepting `"hackrf"` or `"plutosdr"` (matches DEVICE_PROFILES driver keys exactly). Dispatch via `_CAPTURE_DISPATCH` validation dict + explicit if/elif for argument shaping; unknown device raises ValueError before any hardware call. `bandwidth_hz: float | None = None` parameter recorded as SigMF `core:bandwidth` capture-record metadata only - no software DSP, no cropping, no decimation. Pluto passes it to the analog RF filter; HackRF logs a warning and records it as operator intent. `_DEVICE_HW_NAMES` dict deleted; `meta.hw` now read from `DEVICE_PROFILES[device]["display_name"]` (single source of truth). 12 existing tests unchanged; 12 new tests in TestCaptureAndSave. Track A debt clearance (same working tree): two pre-existing test-fixture gaps in AGENTS.md Known Tech Debt rows 618-619 now RESOLVED. Row 618: `test_wipe_flag_deletes_collection` mock expanded from 1 attribute to 5 mirroring every `_parse_args()` default. Row 619: `test_adsb_sweep_uses_max_hold_trace` mocks `builtins.input` via monkeypatch. +14 pytest, +0 Vitest, cleared 2 pre-existing failures.

**Current total:** 1214 passing (837 pytest + 377 Vitest), 0 failures.
