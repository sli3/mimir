...
## Mimir bootstrap note

This is the primary bootstrap step for feeding Stitch an understanding of
Mimir's *existing* dashboard, per the project's decision to work from what
already exists rather than generate from nothing. Target for the first run:
`dashboard/frontend/`. This skill never calls a Stitch MCP tool during its
core work (Phases 1–3) — it reads source files and writes
...
- Access to the frontend project's source directory (`dashboard/frontend/`)
...
design-token libraries. This context tells you *where* to look for styling
information. For Mimir, expect plain React + Vite, no component library
(no Chakra/MUI/Ant confirmed as of this port) — verify against the real
...
> **Compatibility gap — flagged, not resolved:** the upstream skill points to
> five framework-specific reference files
> (`references/react-tailwind.md`, `references/vue.md`,
> `references/svelte.md`, `references/angular.md`,
> `references/plain-css.md`) that were **not included** in what was ported
> here — only the orchestrating `SKILL.md` text was available to port from.
> For Mimir, the relevant one would be `react-tailwind.md`. Until that file
> is sourced and added at `.opencode/skills/extract-design-md/references/`,
> the agent following this skill should proceed using the general extraction
> patterns in Phase 2 below (which are framework-agnostic enough to work
> without the reference) rather than blocking on a file that doesn't exist.
> Do not fabricate the contents of the missing reference file.

Consult the appropriate reference for extraction patterns before proceeding,
if available.
...
- Map component usage: Which heading level do dashboard panels use? What
  about the waterfall/radar/vectordb page headers?
...
Identify 1-2 components unique to this project and describe their styling
patterns. For Mimir, likely candidates: the waterfall display, the AI
Reasoning panel, the ADS-B aircraft table, or the vector-space 3D scatter
controls — pick whichever the target page actually uses.
...
> **Compatibility gap — flagged, not resolved:** the upstream skill requires
> a specific YAML frontmatter block at the top of the output file (with a
> `name` and `colors` mapping) and points to a concrete example at
> `examples/DESIGN.md` as the authoritative template. That example file was
> **not included** in what was ported here. The upstream instructions state
> plainly that omitting this YAML block "is a failure to use this skill
> correctly" — so this is a real gap, not a cosmetic one. Until the real
> example file is sourced, whoever runs this skill should construct the
> frontmatter by hand (a `name:` field matching the project, and a `colors:`
> mapping of the descriptive color names extracted in Phase 2 to their hex
> values) and flag the output for a manual check before any downstream skill
> (`manage-design-system`, `enhance-prompt`) tries to parse it.

Use the markdown section structure below:
...
1. Hand off to the `manage-design-system` skill for the MCP create/update calls
   (not yet ported to this project as of this port — see plan doc)
...
- **Read comments and commit messages.** Developers often document design
  intent in code comments and commit messages. These are gold for
  understanding the *why*.
- **Check for design-token libraries.** If the project uses a token library,
  those files are the most authoritative source of design values.
- **Theme files are higher-signal than component styles.** A theme file that
...
- **Tailwind config is a design system, if present.** If the project has a
  customized `tailwind.config.js`, extract from it first, then spot-check
  components for overrides.
- **CSS custom properties are intentional.** If a developer defined a
  `--brand-primary`-style variable, they're telling you this is a design
  token. Respect that.

## Compatibility notes (OpenCode / Mimir)

- No `[prefix]:tool_name`-style calls existed in the original body to fix —
  this skill's core work (Phases 1–3) never touches the Stitch MCP directly,
  so no tool-name translation was needed for the main workflow.
- `Bash`/`Read`/`Write`/`web_fetch` → lowercase OpenCode equivalents;
  `web_fetch` → `webfetch`.
- Two real content gaps are flagged inline above (Phase 1 Step 3 reference
  files; Phase 3 YAML frontmatter example) rather than silently worked
  around — both should be resolved by sourcing the real upstream files
  before this skill is trusted for unattended use.
