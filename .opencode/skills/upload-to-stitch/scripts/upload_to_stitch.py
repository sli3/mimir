...
MIMIR / OPENCODE NOTE:
    This script itself is unchanged from upstream — it is credential-agnostic
    and simply accepts --api-key as a plain string argument. The calling
    convention for this project sources that value from the
    GOOGLE_STITCH_API_KEY environment variable (already used for this
    project's Stitch MCP connection via opencode.json's {env:} interpolation)
    rather than reading it out of any agent config file. See this skill's
    SKILL.md for the correct invocation pattern.
"""
