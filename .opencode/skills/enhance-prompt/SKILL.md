...
Look for a `.stitch/DESIGN.md` file in the current project (this is where the
`design-md` skill writes it for this project — see that skill's SKILL.md for
...
**Optional file output:** If the user requests, write to a file with a
project-relevant name (e.g. `next-prompt.md`) in a scratch location — this
project does not use the `stitch-loop` skill's multi-page batching workflow,
so there is no fixed consumer for this file. Confirm the destination with the
...
## Compatibility note (OpenCode / Mimir)

This skill makes no direct MCP tool calls (`allowed-tools` is limited to
`read`/`write`), so it required no tool-name translation — unlike
`design-md`, which calls Stitch MCP tools directly and needed its call
syntax rewritten for this project's confirmed `stitch_*` naming. The only
changes from the upstream version are: lowercase tool names in `allowed-tools`
for casing consistency, the DESIGN.md path pointer aligned to
`.stitch/DESIGN.md`, and the `stitch-loop` file-output reference softened
since that skill is not in use on this project.
