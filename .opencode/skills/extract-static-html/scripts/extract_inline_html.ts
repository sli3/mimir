 ...
 *
 * MIMIR / OPENCODE NOTE: this is the "last resort" fallback path — per
 * extract-static-html/SKILL.md, Strategy A (snapshot.ts, live Puppeteer
 * capture) is preferred whenever the app can run locally. This script is
 * only relevant if Mimir's dashboard cannot be run at all. Ported verbatim,
 * no code changes needed — includes real SSRF protection (isSafeUrl) that
 * blocks outbound requests to localhost/private IP ranges, which matters
 * since it parses image URLs out of HTML/JSX source files.
 */
