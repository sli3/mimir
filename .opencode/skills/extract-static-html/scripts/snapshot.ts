 ...
 *
 * MIMIR / OPENCODE NOTE:
 *   This script is ported verbatim — no code changes were needed. Two things
 *   worth knowing before use on this project:
 *   1. It launches a real headless Chrome process via Puppeteer (--no-sandbox
 *      is a standard container flag, not a red flag on its own, but this is
 *      a meaningfully heavier dependency than anything else in this
 *      integration — requires `puppeteer` installed, a full browser binary).
 *   2. It supports an --auth-script flag that dynamically imports and
 *      executes an arbitrary local JS/TS module against the live page. This
 *      project's dashboard has no known auth wall, so this skill's
 *      SKILL.md deliberately does not document or recommend using that flag
 *      — it remains available in the script itself but is out of scope for
 *      Mimir's documented usage.
 */
    ...
    const additionalStats = await page.evaluate(async () => {
      ...
      await processInBatches(objects, CONCURRENCY, async (obj: HTMLObjectElement) => {
        ...
        try {
          ...
          if (contentLength && Number(contentLength) > 500000) return;
          const blob = await resp.blob();
